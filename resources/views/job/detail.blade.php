...
@include('includes.inner_page_title', ['page_title'=>__('Détail de l\'emploi')]) 
    ...
    <div class="container"> 
        ...
        <div class="job-header">
            <div class="jobinfo">
                        ...
                        <div class="ptext">{{__('Date postée')}}: {{$job->created_at->format('M d, Y')}}</div>
						
						@if(!Auth::user() && !Auth::guard('company')->user())
                        <a href="{{route('login')}}"><i class="fa fa-sign-in" aria-hidden="true"></i> {{__('Connectez-vous pour voir le salaire')}} </a>
						@else
			...
			<!-- Job Detail start -->
                <div class="jobmainreq">
                    <div class="jobdetail">
                       <h3><i class="fa fa-align-left" aria-hidden="true"></i> {{__('Détail de l\'emploi')}}</h3>
						
							
							 <ul class="jbdetail">
                            <li class="row">
                                <div class="col-md-4 col-xs-5">{{__('Lieu de travail')}}:</div>
                            ...
                            <li class="row">
                                <div class="col-md-4 col-xs-5">{{__('Société')}}:</div>
                            ...
                            <li class="row">
                                <div class="col-md-4 col-xs-5">{{__('Niveau de carrière:')}}:</div>
                                <div class="col-md-8 col-xs-7"><span>{{$job->getCareerLevel('career_level')}}</span></div>
                            </li>
								<li class="row">
                                <div class="col-md-4 col-xs-5">{{__('Postes')}}:</div>
                            ...
                            <li class="row">
                                <div class="col-md-4 col-xs-5">{{__('Sexe')}}:</div>
                                <div class="col-md-8 col-xs-7"><span>{{$job->getGender('gender')}}</span></div>
                            </li>
                            <li class="row">
                                <div class="col-md-4 col-xs-5">{{__('Diplome')}}:</div>
                                <div class="col-md-8 col-xs-7"><span>{{$job->getDegreeLevel('degree_level')}}</span></div>
                            </li>
                            <li class="row">
                                <div class="col-md-4 col-xs-5">{{__('Appliquer maintenant')}}:</div>
            ...
            <div class="jobButtons">
                <a href="{{route('email.to.friend', $job->slug)}}" class="btn"><i class="fa fa-envelope" aria-hidden="true"></i> {{__('Envoie à un ami')}}</a>
                @if(Auth::check() && Auth::user()->isFavouriteJob($job->slug)) <a href="{{route('remove.from.favourite', $job->slug)}}" class="btn"><i class="fa fa-floppy-o" aria-hidden="true"></i> {{__('Ajouter aux favories')}} </a> @else <a href="{{route('add.to.favourite', $job->slug)}}" class="btn"><i class="fa fa-floppy-o" aria-hidden="true"></i> {{__('Ajouter aux favories')}}</a> @endif
                <a href="{{route('report.abuse', $job->slug)}}" class="btn report"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> {{__('Signaler un abus')}}</a>
            </div>
        </div>
                ...
                <div class="job-header">
                    <div class="contentbox">
                        <h3><i class="fa fa-file-text-o" aria-hidden="true"></i> {{__('Description de l\'emploi')}}</h3>
                        <p>{!! $job->description !!}</p>                       
                    </div>
                </div>
				...
				<div class="job-header benefits">
                    <div class="contentbox">
                        <h3><i class="fa fa-file-text-o" aria-hidden="true"></i> {{__('Avantages')}}</h3>
                        <p>{!! $job->benefits !!}</p>                       
                    </div>
                </div>
				...
				<div class="job-header">
                    <div class="contentbox">                        
                        <h3><i class="fa fa-puzzle-piece" aria-hidden="true"></i> {{__('Compétences requises')}}</h3>
                        <ul class="skillslist">
            ...
            <div class="col-lg-5"> 
				...
				@if($job->isJobExpired())
                <span class="jbexpire"><i class="fa fa-paper-plane" aria-hidden="true"></i> {{__('Le travail a expiré')}}</span>
                @elseif(Auth::check() && Auth::user()->isAppliedOnJob($job->id))
                <a href="javascript:;" class="btn apply applied"><i class="fa fa-paper-plane" aria-hidden="true"></i> {{__('Déjà appliqué')}}</a>
                @else
                <a href="{{route('apply.job', $job->slug)}}" class="btn apply"><i class="fa fa-paper-plane" aria-hidden="true"></i> {{__('Appliquer maintenant')}}</a>
                @endif
				</div>
				
				
				<div class="companyinfo">
					<h3><i class="fa fa-building-o" aria-hidden="true"></i> {{__('Présentation de l\'entreprise')}}</h3>
                            ...
                            <div class="opening">
                                <a href="{{route('company.detail',$company->slug)}}">
                                    {{App\Company::countNumJobs('company_id', $company->id)}} {{__('Offres d\'emploi actuelles')}}
                                </a>
                            </div>
				...
				<div class="companyoverview">
					...
					<p>{{\Illuminate\Support\Str::limit(strip_tags($company->description), 250, '...')}} <a href="{{route('company.detail',$company->slug)}}">Lire la suite</a></p>
					</div>
                        </div>
