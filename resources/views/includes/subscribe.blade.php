  ...
  <div class="container">
    ...
    <div class="row">
		...
		<div class="col-lg-4">
		 ...
		 <h6>{{__('Bulletin')}}</h6> 
    <p>{{__('Abonnez-vous à notre newsletter et restez informé.')}}</p>	
      ...
      {{ csrf_field() }}
		  ...
		  <input type="text" class="form-control" placeholder="{{__('Nom')}}" name="name" id="name" required="required">
			<input type="text" class="form-control" placeholder="{{__('Email')}}" name="email" id="email" required="required">
		  <div class="input-group-append">
			<button class="btn btn-default" type="button" id="send_subscription_form">{{__('S\'abonner')}}</button>			  
		  </div>
		</div> 
